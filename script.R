...
tcells <- FindVariableFeatures(tcells, selection.method = "vst", nfeatures = 2000) #se si usa il default, non serve specificare il numero
...
plot1 <- VariableFeaturePlot(tcells) #plot with stardardized variance, in red the top variable genes
...
all.genes <- rownames(tcells)  #all the genes, se fai l'analisi solo sui gene ipervariabili puoi perdere info importanti (il gene di interesse può non essere variabile tr ai cluster)
tcells <- ScaleData(tcells, features = all.genes)

#Run PCA

tcells <- RunPCA(tcells, features = VariableFeatures(object = tcells)) #to make PCA, varibale feature

# Examine and visualize PCA results a few different ways
print(tcells[["pca"]], dims = 1:5, nfeatures = 5) #isolates most 5 genes for all the dimension of PCA (positive and negative correlation)
...
JackStrawPlot(tcells, dims = 1:20) #tutti significati tranne il 15
ElbowPlot(tcells) #15 is elbow point
dev.off()
...
tcells.markers2<-FindMarkers(tcells,ident.1="0",ident.2 = c("1","2"),only.pos="T")
