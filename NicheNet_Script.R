...
## Sender: MAIT cells
Idents(MAIT) = 'time'
expressed_genes_sender = get_expressed_genes("P3", MAIT, pct = 0.10) #perch� mi interesssa solo timepoint p3 e geni molto espressi
expressed_genes_sender <- expressed_genes_sender[expressed_genes_sender %in% rownames(ligand_target_matrix)] #select genes that are in ligand target matrix
...
background_expressed_genes =  expressed_genes_receiver[expressed_genes_receiver %in% colnames(ligand_target_matrix)]
